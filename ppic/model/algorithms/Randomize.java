...
public class Randomize extends Algorithm
{
	...
	public Expression getRandomExpression (double rate)
	{
		...
		//return zero argument expression
		if(randDecimal < rate){
		...
		if(nextRand < rate){
			Expression randomChild = apply(rate*1.33);
			Expression oneArg = makeOneArgumentExpression(randomChild);
			return oneArg;
		}
		else {
			Expression randomChildOne = apply(rate*1.33);
			Expression randomChildTwo = apply(rate*1.33);
			Expression twoArg = makeTwoArgumentExpression(randomChildOne,randomChildTwo);
			return twoArg;
		}
	}
